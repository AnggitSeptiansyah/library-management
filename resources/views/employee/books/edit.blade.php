...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <form action="{{ route('employee.books.update', $book) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="publication_year" value="{{ old('publication_year', $book->publication_year) }}" min="1900" max="{{ date('Y') + 1 }}"
                ...
                <div>
                    ...
                    <input type="number" name="total_pages" value="{{ old('total_pages', $book->total_pages) }}" min="1"
                ...
                <div>
                    ...
                    <input type="number" name="total_copies" value="{{ old('total_copies', $book->total_copies) }}" min="{{ $book->total_copies - $book->available_copies }}" required
                    ...
                    <p class="mt-1 text-xs text-gray-500">Currently borrowed: {{ $book->total_copies - $book->available_copies }}</p>
                    @error('total_copies')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="w-32 h-40 object-cover rounded">
                </div>
                @endif

                <!-- Cover Image -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ $book->cover_image ? 'Change Cover Image' : 'Cover Image' }}</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $book->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
